...
class AvatarClickHandler:
    ...
    def handle_avatar_click(avatar_control: auto.Control,
                            tab_control_name: str = "cdn_搜索",model:bool=True) -> Dict[str, Any]:
        """
        处理头像点击事件并判断页面类型
        ...
        try:
            ...
            if len(tabs) == 4:
                ...
                """
                这里有个问题，就是从全局找，会遍历所有的控件，会非常耗时
                """
                # 查找视频号控件
                #全局搜索耗时太长100多秒,不友好
                temp_item_video_contorl_name=tabs[-1].Name
                temp_item_video_contorl=auto.DocumentControl(Name=temp_item_video_contorl_name)
                video_account_1 = auto.TextControl(searchFromControl=temp_item_video_contorl,Name="视频号",foundIndex=1)
                if video_account_1.Exists(maxSearchSeconds=2):
                    video_account_2 = auto.TextControl(searchFromControl=temp_item_video_contorl,Name="视频号", foundIndex=2)
        ...
        library_logger.debug(f"result{result}")
        return result
